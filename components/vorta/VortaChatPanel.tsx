...
// Chat behavior extracted verbatim from the original VortaWidget — the
// Copilot-ready panel: local FAQ + /api/ai/guide fallback, daily limit,
// suggestion chips. The floating mascot (VortaStage) controls `open`.
...
export function VortaChatPanel({
  dict,
  lang,
  aiEnabled,
  open,
  ...
  // On open: greet and seed the welcome message (same behavior as before).
  useEffect(() => {
    if (open) {
  ...
  return (
    <div className="fixed inset-x-2 bottom-2 top-[4.25rem] z-50 flex flex-col overflow-hidden rounded-2xl border border-navy-100 bg-white shadow-card-hover animate-fade-up sm:inset-auto sm:bottom-5 sm:right-5 sm:h-[600px] sm:max-h-[calc(100dvh-6rem)] sm:w-[392px] print:hidden">
      ...
      {/* Stage controls (minimize / hide / presentation) */}
      {controls}
